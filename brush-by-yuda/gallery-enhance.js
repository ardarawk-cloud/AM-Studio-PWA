(function(){
  ...
  Promise.all([0,1,2,3,4].map(function(i){
    return fetch(base+'gallery-atlas-q0'+i+'.txt',{cache:'force-cache'}).then(function(response){
      if(!response.ok) throw new Error('Gallery source unavailable');
      return response.text();
    });
  })).then(function(parts){
    const atlas='data:image/jpeg;base64,'+parts.join('').replace(/\s+/g,'');
    ...
    function render(cat){
      ...
      items.filter(function(x){return cat==='All'||x[0]===cat;}).forEach(function(item){
        ...
        const caption=document.createElement('figcaption');caption.innerHTML='<small>'+category+'</small><strong>'+title+'</strong>';
    ...
    cats.forEach(function(cat,i){
      ...
      button.className='byFilter'+(i===0?' active':'');
      button.type='button';
    ...
    console.error('Brush gallery image load failed',error);
  });
})();
