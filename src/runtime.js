...
function metaEnv(env){
  ...
  return new Proxy(env,{get(target,prop){if(prop==='META_SYSTEM_USER_TOKEN')return target.META_PAGE_ACCESS_TOKEN||target.META_SYSTEM_USER_TOKEN;return target[prop]}});
}
...
function episodeNo(message=''){
  const m=String(message).match(/(?:episode|ep(?:isode)?|eps?|chapter|bab)\s*[-:#.]?\s*0*(\d{1,4})\b/i);
  return m?Number(m[1]):null;
}
function titleOf(message=''){
  const lines=String(message).split(/\r?\n/).map(x=>x.trim()).filter(Boolean);
  if(!lines.length)return null;
  const t=lines[0].replace(/^(?:episode|ep(?:isode)?|eps?|chapter|bab)\s*[-:#.]?\s*0*\d{1,4}\s*[-–—:|]?\s*/i,'').trim();
...
async function directPageAudit(env,pageId,mapping,maxPosts=100){
  ...
  const rows=(posts.data?.data||[]).slice(0,maxPosts).map(p=>({
    postId:p.id,
    publishedAt:p.created_time||null,
    permalink:p.permalink_url||null,
    message:p.message||'',
    title:titleOf(p.message||''),
    episodeNumber:episodeNo(p.message||''),
    coverCandidate:/\bcover\b|sampul|official cover|cover resmi|main cover/i.test(p.message||''),
    fullPicture:p.full_picture||null,
    characterAppearances:[],
    provenance:'FB_DETECTED_DIRECT_PAGE_ID'
  }));
  const eps=[...new Set(rows.map(x=>x.episodeNumber).filter(Number.isFinite))].sort((a,b)=>a-b);
  return{
    ok:true,pageId,pageName:profile.data?.name||mapping.title,mapping,
    discoverySource:'DIRECT_PAGE_ID_FALLBACK',scannedPosts:rows.length,
    detectedEpisodes:eps,highestDetectedEpisode:eps.length?eps[eps.length-1]:null,
    coverCandidates:rows.filter(x=>x.coverCandidate).map(x=>({postId:x.postId,publishedAt:x.publishedAt,permalink:x.permalink,fullPicture:x.fullPicture,title:x.title})),
    posts:rows
  };
}
async function directDiscovery(env,maxPosts=100){
  const token=activeToken(env);
  if(!token)return{ok:false,error:'META_TOKEN_NOT_CONFIGURED'};
  const audits=[];
  for(const [pageId,mapping] of Object.entries(PAGE_MAP))audits.push(await directPageAudit(env,pageId,mapping,maxPosts));
  const successful=audits.filter(x=>x.ok);
  return{
    ok:true,generatedAt:new Date().toISOString(),controller:'ARDA_ACC_HUB',
    discoverySource:'DIRECT_PAGE_ID_FALLBACK',mappedPages:successful.length,
    attemptedPages:audits.length,failedPages:audits.filter(x=>!x.ok).map(x=>({pageId:x.pageId,title:x.mapping?.title,error:x.error,metaError:x.metaError})),
    audits:successful
  };
}
async function readSnapshot(env){
  if(!env.COMIC_ASSETS)return null;
  const o=await env.COMIC_ASSETS.get(SNAPSHOT_KEY);if(!o)return null;
  try{return JSON.parse(await o.text())}catch{return null}
}
async function saveSnapshot(env,s){
  if(!env.COMIC_ASSETS)return false;
  await env.COMIC_ASSETS.put(SNAPSHOT_KEY,JSON.stringify(s),{httpMetadata:{contentType:'application/json'}});return true;
}
...
async function runtimeCatalog(request,env){
  const assetUrl=new URL('/catalog.json',request.url);const res=await env.ASSETS.fetch(new Request(assetUrl.toString()));if(!res.ok)return res;
  const catalog=await res.json(),snapshot=await readSnapshot(env);if(!snapshot)return Response.json(catalog,{headers:{'cache-control':'no-store','x-am-canon-source':'STATIC_NO_SNAPSHOT'}});
  const index=buildPublishedIndex(snapshot),byId=Object.fromEntries(index.series.map(s=>[s.seriesId,s]));
  catalog.series=(catalog.series||[]).map(s=>{const fb=byId[s.id];if(!fb)return s;return{...s,facebookAudit:{pageId:fb.pageId,pageName:fb.pageName,detectedEpisodes:fb.detectedEpisodes,detectedEpisodeCount:fb.detectedEpisodeCount,highestDetectedEpisode:fb.highestDetectedEpisode,sequenceContiguousFromOne:fb.sequenceContiguousFromOne,verificationState:fb.verificationState,coverCandidates:fb.coverCandidates},status:s.episodeCountVerified?s.status:(fb.detectedEpisodeCount?'FB_AUDIT_REVIEW_READY':'FB_CANON_AUDIT_PENDING')}});
  catalog.studio={...catalog.studio,facebookCanonSnapshot:index.generatedAt||null};return Response.json(catalog,{headers:{'cache-control':'no-store','x-am-canon-source':'FB_SNAPSHOT_OVERLAY'}});
}

export default{
  async fetch(request,env,ctx){
    const url=new URL(request.url);
    if(url.pathname==='/api/meta/token-source'&&request.method==='GET')return Response.json({ok:true,pageAccessTokenConfigured:Boolean(env.META_PAGE_ACCESS_TOKEN),legacySystemTokenConfigured:Boolean(env.META_SYSTEM_USER_TOKEN),activeTokenSource:env.META_PAGE_ACCESS_TOKEN?'META_PAGE_ACCESS_TOKEN':env.META_SYSTEM_USER_TOKEN?'META_SYSTEM_USER_TOKEN':'NONE'},{headers:{'cache-control':'no-store'}});
    ...
    if(url.pathname==='/api/meta/canon-sync'&&request.method==='POST'){
      ...
      if(!env.AM_STUDIO_ADMIN_KEY||key!==env.AM_STUDIO_ADMIN_KEY)return Response.json({ok:false,error:'ADMIN_AUTH_REQUIRED'},{status:401});
      let body={};try{body=await request.json()}catch{}
      const result=await directDiscovery(env,Math.max(1,Math.min(100,Number(body.maxPosts||100))));
      if(!result.ok)return Response.json(result,{status:503});
      const saved=await saveSnapshot(env,result);return Response.json({...result,saved},{headers:{'cache-control':'no-store'}});
    }
    if(url.pathname==='/api/meta/pages'&&request.method==='GET'){
      const d=await directDiscovery(env,1);return Response.json({ok:d.ok,data:(d.audits||[]).map(a=>({id:a.pageId,name:a.pageName,mapping:a.mapping,discoverySource:a.discoverySource})),failedPages:d.failedPages||[],controller:'ARDA_ACC_HUB',scope:'AM_STUDIO_ONLY'},{headers:{'cache-control':'no-store'}});
    }
  ...
  async scheduled(controller,env,ctx){
    ctx.waitUntil((async()=>{const d=await directDiscovery(env,50);if(d.ok)await saveSnapshot(env,d)})());
