...
async function uploadAsset(request,env){
  ...
  await env.COMIC_ASSETS.put(key,request.body,{httpMetadata:{contentType:type,cacheControl:'public, max-age=31536000, immutable'},customMetadata:{source:'OWNER_ADMIN_UPLOAD',uploadedAt:new Date().toISOString()}});
  const head=await env.COMIC_ASSETS.head(key);
  return Response.json({ok:true,key,size:head?.size??length,contentType:head?.httpMetadata?.contentType||type},{headers:{'cache-control':'no-store'}});
}
...
async function saveMeta(request,env){
  ...
  await env.COMIC_ASSETS.put(key,JSON.stringify(meta),{httpMetadata:{contentType:'application/json',cacheControl:'no-store'},customMetadata:{source:'OWNER_ADMIN_META'}});
...
async function buildInventory(env){
  ...
  const objects=await listAll(env,'comics/'),covers=new Set(),groups=new Map(),metaObjects=[];
  for(const o of objects){
    const c=o.key.match(/^comics\/([a-z0-9-]+)\/cover\./i);if(c){covers.add(c[1]);continue}
    const p=o.key.match(/^comics\/([a-z0-9-]+)\/ep(\d{3})\/page-(\d{2,3})\./i);if(p){const k=`${p[1]}:${Number(p[2])}`;if(!groups.has(k))groups.set(k,{seriesId:p[1],episode:Number(p[2]),pages:new Map(),meta:null});groups.get(k).pages.set(Number(p[3]),`/media/${o.key}`);continue}
    if(META_KEY.test(o.key))metaObjects.push(o.key);
  }
  for(const key of metaObjects){
    const m=key.match(META_KEY);if(!m)continue;const k=`${m[1]}:${Number(m[2])}`;if(!groups.has(k))groups.set(k,{seriesId:m[1],episode:Number(m[2]),pages:new Map(),meta:null});
...
function mergeReader(staticRegistry,inventory){
  ...
  for(const g of inventory.episodes){
    if(!g.meta&&!g.pages.size)continue;const d=dynamicEntry(g,inventory.covers),k=`${d.seriesId}:${d.episode}`,old=map.get(k);
    map.set(k,old?{...old,...d,pageCount:Math.max(Number(old.pageCount)||0,Number(d.pageCount)||0),title:old.title||d.title}:d);
  }
  out.registry={...(out.registry||{}),version:Number(out.registry?.version||0)+1,storage:'R2_COMIC_ASSETS',runtimeOverlay:true};out.episodes=[...map.values()].sort((a,b)=>a.seriesId.localeCompare(b.seriesId)||Number(a.episode)-Number(b.episode));return out;
}
...
async function listAssets(request,env){
  ...
  const objects=await listAll(env,prefix);return Response.json({ok:true,prefix,count:objects.length,objects:objects.map(o=>({key:o.key,size:o.size,uploaded:o.uploaded?.toISOString?.()||null}))},{headers:{'cache-control':'no-store'}});
}

async function media(request,env,key){
  if(!env?.COMIC_ASSETS)return new Response('Comic storage unavailable',{status:503});
  const clean=cleanKey(key);if(!clean)return new Response('Not found',{status:404});const obj=await env.COMIC_ASSETS.get(clean);if(!obj)return new Response('Not found',{status:404,headers:{'cache-control':'no-store'}});
  const h=new Headers();if(typeof obj.writeHttpMetadata==='function')obj.writeHttpMetadata(h);if(!h.get('content-type'))h.set('content-type','application/octet-stream');h.set('cache-control','public, max-age=31536000, immutable');if(obj.httpEtag)h.set('etag',obj.httpEtag);h.set('x-am-asset-source','R2_COMIC_ASSETS');return new Response(obj.body,{headers:h});
}

async function jsonOverlay(request,env,kind){
  const response=await base.fetch(request,env);if(!response.ok)return response;let data;try{data=await response.json()}catch{return response}
  const inventory=await buildInventory(env),merged=kind==='reader'?mergeReader(data,inventory):mergeCatalog(data,inventory);return Response.json(merged,{headers:{'cache-control':'no-store'}});
}

async function injectAdmin(response){
  const ct=response.headers.get('content-type')||'';if(!response.ok||!ct.includes('text/html'))return response;const html=await response.text(),src='/admin-panel.js?v=20260813b',marker=`<script src="${src}" defer></script>`;
  let out=html.replace(/<script src="\/asset-upload\.js\?v=[^"]+" defer><\/script>/g,'').replace(/<script src="\/admin-panel\.js\?v=[^"]+" defer><\/script>/g,'');out=out.includes('</body>')?out.replace('</body>',`${marker}</body>`):`${out}${marker}`;const h=new Headers(response.headers);h.set('cache-control','no-store');return new Response(out,{status:response.status,statusText:response.statusText,headers:h});
}
...
export default{
  async fetch(request,env,ctx){
    ...
    if(url.pathname==='/reader-assets.json'&&request.method==='GET')return jsonOverlay(request,env,'reader');
    if(url.pathname==='/catalog.json'&&request.method==='GET')return jsonOverlay(request,env,'catalog');
    const response=await base.fetch(request,env,ctx);return injectAdmin(response);
  },
