import base from './canon-hold-runtime.js';
...
function fail(error,status=500){
  return Response.json({ok:false,error:String(error?.message||error)},{status,headers:{'cache-control':'no-store'}});
}
  ...
  async fetch(request,env,ctx){
    ...
    if(url.pathname==='/api/divisions'&&request.method==='GET'){
      ...
      try{
        ...
        return Response.json({ok:true,...registry},{headers:{'cache-control':'no-store','x-am-brain':'comic-division-v1'}});
    ...
    if(match&&request.method==='GET'){
      ...
      try{
        ...
        return Response.json({ok:true,context},{headers:{'cache-control':'no-store','x-am-brain':'comic-division-v1'}});
